...
export default function HandRanks() {
  ...
  useEffect(() => {
    ...
    // initialise on mount and listen for changes
    handleChange(mq);
  ...
  return (
    <aside className={`handranks ${collapsed ? "is-collapsed" : ""}`}>
      <button
        type="button"
        className="handranks__collapseToggle"
        onClick={() => setCollapsed((value) => !value)}
        aria-expanded={!collapsed}
        aria-label={collapsed ? "Show hand rankings" : "Hide hand rankings"}
      >
        <span className="handranks__collapseIcon" aria-hidden="true">
          {collapsed ? "▶" : "◀"}
        </span>
        <span className="handranks__collapseLabel">Hand Rankings</span>
      </button>

      {!collapsed && (
        <div className="handranks__content">
